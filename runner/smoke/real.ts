 ...
 *
 * Phase B (runner-process level): main() boot, GET /status, POST /prompt -> SSE
...
async function phaseA(): Promise<{ workDir: string; agentDir: string; sessionDir: string }> {
  ...
  // 3. invalid model -> clean PromptError (maps to HTTP 400 in the runner)
...
async function phaseB(): Promise<void> {
  console.log('[B] runner-process check (real runner, no fake, no credentials)');
  ...
  const app = await withTimeout(main(process.env), 30_000, 'runner boot');
  let closeSse: (() => void) | undefined;
  try {
    const address = app.server.address();
    expect(address !== null && typeof address === 'object', 'runner has tcp address');
    ...
    const status = (await statusResponse.json()) as { busy: boolean };
