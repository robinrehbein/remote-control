...
- **The permission gate protects against mistakes, not against a determined agent.** In `ask` mode the runner holds write/exec tool calls until a device approves them at `/permissions/<id>`. That gate authenticates with the session API token (`SHIM_TOKEN`). The runner now deletes `SHIM_TOKEN` from `process.env` right after start-up, and the egress-proxy credential injected into `HTTP(S)_PROXY` is a *derived, different* value (`proxyTokenFor`), so a Bash child of the agent can no longer read the API token from its environment and self-approve via `curl 127.0.0.1:8080`. This raises the bar but is not an airtight boundary: the agent runs as the same uid as the runner and could still reach the token through other means (process memory, the runner's own file descriptors). The gate is a guardrail against accidental destructive actions, not a containment boundary against an agent that is actively trying to escape it — for that, rely on the network policy (allowlist/isolated) and the single-tenant deployment model.
