...
- **Proxy-Zwang im Shim**: `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` allein genügen nicht — Node beachtet diese Variablen von sich aus nicht (weder `fetch`/undici noch `http.request`). Jeder Shim setzt deshalb beim Prozessstart undicis **globalen Dispatcher** auf einen `EnvHttpProxyAgent` (`shims/*/src/proxy.ts`, erster Import in `src/index.ts`), der Orchestrator injiziert zusätzlich `NODE_USE_ENV_PROXY=1` für Nodes eigene Clients und für Node-Kindprozesse. Ohne Proxy-Variablen (Policy `open`) wird nichts installiert, der Container spricht direkt.
...
- **Ehrlich über die Grenzen** (Details: `SECURITY.md`):
  ...
  - Der globale undici-Dispatcher wirkt nur im **Shim-Prozess** (pi-SDK, GitHub-API-Calls der Gitops). Agent-Runtimes, die als eigener Prozess laufen — `codex`, `junie`, `kilo serve`, die Claude-CLI — müssen die Proxy-Variablen selbst beachten; sie erben Env inkl. `NODE_USE_ENV_PROXY=1` (deckt Node-Kindprozesse ab), für Nicht-Node-Runtimes hängt es an deren HTTP-Client.
