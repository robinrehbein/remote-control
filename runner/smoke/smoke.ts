...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'pi-runner-smoke-work-'));
  const agentDir = await mkdtemp(join(tmpdir(), 'pi-runner-smoke-agent-'));
  ...
  expect(lastId >= 3, `the runner has sequenced several events by now (lastId=${lastId})`);
  ...
  // --- provider without model -> the pi default, not a 400 ---
  ...
  const credsDir = await mkdtemp(join(tmpdir(), 'pi-runner-smoke-creds-'));
  ...
  if (gitAvailable) {
    const e2eRoot = await mkdtemp(join(tmpdir(), 'pi-runner-smoke-e2e-'));
    try {
