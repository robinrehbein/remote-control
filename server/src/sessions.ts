...
import { setEgressTokenValidator } from './egress-proxy.js';
...
export class SessionManager {
  ...
  constructor(
    ...
    // Egress proxy auth: only tokens matching a live session's shim_token pass (read-only lookup).
    setEgressTokenValidator((t) => this.store.listSessions(TENANT).some((r) => r.shim_token === t));
  }
  ...
  createSession(msg: CreateMsg, tenant: string = TENANT): SessionRow {
    ...
    if (docker.isRemote()) {
      if (networkPolicy === 'allowlist' || networkPolicy === 'isolated') {
        throw new Error('network policies require local docker socket mode');
      }
