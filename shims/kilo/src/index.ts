...
function resolveOpenCodeBin(): string {
  ...
  const pkgPath = require.resolve('@kilocode/cli/package.json');
  const pkg = JSON.parse(readFileSync(pkgPath, 'utf8')) as { bin?: string | Record<string, string> };
  const rel = typeof pkg.bin === 'string' ? pkg.bin : pkg.bin?.kilo ?? pkg.bin?.kilocode;
  if (typeof rel !== 'string') throw new Error('@kilocode/cli kilo bin entry not found');
 ...
 * shim's GET /models returns; an empty string falls back to kilo's default.
 */
